...
class Tagtree:
    """
    Tagtree class adds tagtrees related features to the tag classes.

    Expected to be used as mixin in class following VirtualTagsProtocol.
    """

    def get_common_tags_from_files(self: VirtualTagsProtocol, files):
        """
        ...
        for currentfile in files:
            list_of_tags_per_file.append(
                set(self.current_service.extract_tags_from_filename(currentfile))
            )
    ...
    def generate_tagtrees(
        ...
        if filtertags:
            ...
            files = self.filter_files_matching_tags(files, filtertags)
        ...
        tags = self.current_service.get_tags_from_files_and_subfolders(
            startdir=os.getcwd(), use_cache=True, options=options
        )
        ...
        tags_of_files = [
            self.current_service.extract_tags_from_filename(x) for x in files
        ]
        ...
        for currentfile in enumerate(files):
            ...
            if len(tags_of_currentfile) == 0:
                    ...
                    if not options.dryrun:
                        try:
                            create_link(
                                filename,
                                os.path.join(
                                    nontagged_item_dest_dir, basename, options=options
                                ),
                            )
                ...
                if link_missing_mutual_tagged_items:
                    ...
                    for unique_tagset in self.unique_tags:
                        ...
                        if not set(tags_of_currentfile).intersection(
                            set(unique_tagset)
                        ):
                            ...
                            if not options.dryrun:
                                try:
                                    create_link(
                                        filename,
                                        os.path.join(
                                            no_uniqueset_tag_found_dir, basename
                                        ),
                                        options=options,
                                    )
                                except FileExistsError:
