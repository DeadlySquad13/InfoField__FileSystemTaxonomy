...
class TagRepr:
    """
    TagRepr - represents tags in human readable format. Used for beauty printing.

    Expected to be used as mixin in class following VirtualTagsProtocol.
    """

    def find_similar_tags(self: VirtualTagsProtocol, tag, tags):
        """
    ...
    def print_tag_dict(
        ...
        print(
            "\n {0:{1}} : {2:{3}}".format(
                "count", maxlength_count, "tag", maxlength_tags
            )
        )
        print(" " + "-" * (maxlength_tags + maxlength_count + 7))
        for tuple in sorted(
            list(tag_dict.items()), key=operator.itemgetter(sort_index)
        ):
            ...
            if vocabulary and print_similar_vocabulary_tags:
                ...
                similar_tags_list = self.find_similar_tags(
                    tuple[0], tags_for_comparing
                )
    ...
    def print_tag_set(self: VirtualTagsProtocol, tag_set, vocabulary=False, print_similar_vocabulary_tags=False):
        """
    ...
    # - Beautified.
    def get_tag_visual(self: VirtualTagsProtocol, tags_for_visual=None):
        """
