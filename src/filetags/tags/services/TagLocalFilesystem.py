...
from filetags.tags.services.TagService import TagService
from filetags.tags.utils import add_tag_to_countdict


@dataclass()
@TagService.register
class TagLocalFilesystem:
    def contains_tag(self, filename: str, tagname=False):
        """
        ...
        components = re.match(
            FILE_WITH_TAGS_REGEX, os.path.basename(basename_without_lnk)
        )
    ...
    def extract_tags_from_path(self, path: str) -> Tagnames:
        ...
        for item in splitall(abspath):
            itemtags = self.extract_tags_from_filename(item)
            for currentitemtag in itemtags:
                if currentitemtag not in tags:
                    tags.append(currentitemtag)
        return tags

    def adding_tag_to_filename(self, filename: Filename, tagname: Tagname) -> Filename:
        """
        ...
        if self.contains_tag(basename_without_lnk) is False:
            logging.debug(
                "adding_tag_to_filename(%s, %s): no tag found so far"
                % (filename, tagname)
            )

            components = re.match(FILE_WITH_EXTENSION_REGEX, basename_without_lnk)
            if components:
                old_basename = components.group(
                    FILE_WITH_EXTENSION_REGEX_FILENAME_INDEX
                )
        ...
        elif self.contains_tag(basename_without_lnk, tagname):
            logging.debug(
    ...
    def removing_tag_from_filename(self, orig_filename: Filename, tagname: Tagname) -> Filename:
        """
        ...
        if not self.contains_tag(orig_filename, tagname):
        ...
        if not components:
            logging.debug(
                "file [%s] does not match FILE_WITH_TAGS_REGEX" % orig_filename
            )
            return orig_filename
        else:
            tags = components.group(FILE_WITH_TAGS_REGEX_TAGLIST_INDEX).split(
            ...
            if len(tags) < 2:
                ...
                new_filename = (
                    old_filename
                    + FILENAME_TAG_SEPARATOR
                    + BETWEEN_TAG_SEPARATOR.join(
                        [tag for tag in tags if tag != tagname]
                    )
                    + extension
                )
    ...
    # REFACTOR: Add type.
    # For reference: cache_of_files_with_metadata - dict of big list of dicts: 'filename', 'path' and other metadata
    ...
    def get_tags_from_files_and_subfolders(
        ...
        logging.debug(
            "g(TagLocalFilesystem)et_tags_from_files_and_subfolders called with startdir [%s], cached startdirs [%s]"
            % (startdir, str(len(list(self.cache_of_tags_by_folder.keys()))))
        )

        if use_cache and startdir in list(self.cache_of_tags_by_folder.keys()):
            logging.debug(
                "get_tags_from_files_and_subfolders: found "
                + str(len(self.cache_of_tags_by_folder[startdir]))
                + " tags in cache for directory: "
                + startdir
            )
            return self.cache_of_tags_by_folder[startdir]
            ...
            for root, dirs, files in os.walk(startdir):
                ...
                for filename in files:
                    for tag in self.extract_tags_from_filename(filename):
                        tags = add_tag_to_countdict(tag, tags)

                for dirname in dirs:
                    for tag in self.extract_tags_from_filename(dirname):
                        tags = add_tag_to_countdict(tag, tags)
        ...
        if use_cache:
            self.cache_of_tags_by_folder[startdir] = tags
        return tags
